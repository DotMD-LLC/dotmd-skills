# DotMD user guides

These guides are organized by jobs, not menus. Interface labels can evolve; use the closest matching control in your current DotMD client.

## Learn the workspace

- [Getting started](getting-started.md)
- [Files and templates](files-and-templates.md)
- [Keyboard and navigation](keyboard-and-navigation.md)

## Create

- [Docs](docs.md)
- [Slides](slides.md)
- [Sheets](sheets.md)
- [Import and export](import-export.md)

## Work together

- [Collaboration](collaboration.md)
- [Sharing and publishing](sharing-and-publishing.md)
- [History and recovery](history-and-recovery.md)
- [GitHub sync](github-sync.md)

## Work with AI

- [AI and MCP](ai-and-mcp.md)
- [Install skills on five AI platforms](install-ai-platforms.md)

For billing, legal, account recovery, and current plan details, use the help and settings surfaces at [dotmd.co](https://dotmd.co).
